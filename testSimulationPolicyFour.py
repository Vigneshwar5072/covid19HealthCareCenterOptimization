...
def solve_covid19_facility(c_coordinates, cv_19_demand, sick_demand,gamma, alpha, timePeriod,e_capacity, t_capacity,total_permanent_facility_cost,total_temporary_facility_cost):
    ...
    for f in facilities:
        ...
        if(f<=14):
            e_capacity[f] = e_capacity[f] - f_demand[f]+gamma+alpha
...
for t in range(0, 11):
    ...
    if t == 0:
        ...
        counties, time_period, coordinates, sick_forecast  = gp.multidict({
            1: [t,(1, 1.5), random.randint(50,75)],
        })
        e_capacity = 0
        t_capacity = 0
...
y1 = [7900, 7823, 7760, 7685, 7579, 7472, 7249, 6900, 6417, 5805, 2835, 67]

y2 = [7900, 7826, 7766, 7680, 7603, 7511, 7362, 7106, 6699, 5870, 4568, 1355]

y3 = [7900, 7842, 7778, 7714, 7624, 7514, 7371, 7197, 6801, 6389, 5459, 3696]
lines = [y1,y2,y3]
colors  = ['r','g','b']
labels  = ['0.080','0.075','0.070']
...
y1 = [190,    350,    540,    805,   1075,   2450,   5210,  12940,
        23875, 114565, 210130]

y2 = [ 185,    335,    550,    740,    970,   1550,   3575,   8425,
        22565,  53545, 175380]

y3 = [  145,   305,   465,   690,   965,  1480,  2855,  6790, 13400,
       30850, 82600]
